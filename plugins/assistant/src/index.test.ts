...
it('AMD-004/S2 R1-S1 declares exactly the bounded Context model-session policy', async () => {
  ...
  const runModelSession = vi.fn(async () => ({
    turnId: 'turn_alpha', conversationId: 'conv_alpha', status: 'completed' as const, question: 'Which note explains AnthraciteMD?',
    provider: 'openai-codex' as const, model: 'gpt-5.4', createdAt: '2026-07-20T12:00:00.000Z', completedAt: '2026-07-20T12:00:01.000Z',
    answer: 'The AnthraciteMD note does.', error: null, sources: [],
  }))
  let policy: unknown
  ...
  await expect(handler?.({ question: 'Which note explains AnthraciteMD?' }, runModelSession)).resolves.toMatchObject({ turnId: 'turn_alpha' })
  expect(runModelSession).toHaveBeenCalledWith({
    question: 'Which note explains AnthraciteMD?',
    policy: {
      prompt: expect.stringContaining('workspace evidence'),
      tools: ['workspace_search', 'workspace_read'],
    },
...
it('AMD-004/S2 R1-S1 passes shared bundled-plugin conformance', async () => {
