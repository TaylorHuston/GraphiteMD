...
| S2/R2-S3, S2/R2-S4, S2/R3-S1 | `apps/web/src/App.test.tsx` — save recovery, failed-draft empty-history guard, guarded popstate, and post-rename edit cases | Retry and explicit conflict recovery are reachable, cancelled note-to-note or note-to-empty history restores the displayed URL and draft, and renamed resources remain saveable while inventory/history reconcile. | Passing 2026-07-19. |
...
| S3/R1-S3 | `apps/server/tests/http/authentication.test.ts` — `reports a recoverable local-index failure without misclassifying workspace authority`; `apps/web/src/App.test.tsx` — honest recoverable failure states; source inspection of `apps/server/start/routes.ts` | A failed local index becomes a recoverable error with an explicit rebuild action, workspace-authority loss remains distinct, unexpected failures propagate, and workbench selection/draft ownership remains outside the search component. | Passing 2026-07-19; manual draft confirmation remains. |
